...
import datasets
import losses
from utils import Config, get_artifacts
...
if __name__ == "__main__":
    ...
    for probabilities_id, loss_function, alpha, lambda_, trial_idx in tqdm(param_combinations):
        artifacts = get_artifacts(probabilities_id)
        all_softmax = artifacts.all_softmax
        all_labels = artifacts.all_labels
        ...
        if prev_dataset != dataset_name:
            dataset = getattr(datasets, dataset_name)(
                hierarchy_artifacts=artifacts.hierarchy_artifacts,
        ...
        # Only calculate the scores if the lambda, alpha or loss function has changed
        if prev_lambda != lambda_ or prev_alpha != alpha or prev_loss_function != loss_function:
            ...
            loss.distances = pairwise_distances
            ...
            val_metrics_df = pd.DataFrame(
                {
                    ...
                    "entropy": stats.entropy(val_softmax, axis=1),
                    "true_softmax": val_softmax[np.arange(n_validation), val_labels],
                }
            )
            test_metrics_df = pd.DataFrame(
                {
                    ...
                    "entropy": stats.entropy(test_softmax, axis=1),
                    "true_softmax": test_softmax[np.arange(n_test), test_labels],
                }
            )
